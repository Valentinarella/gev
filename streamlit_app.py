...
if view == "Hazard Map":
    ...
    for h in hazards:
        ...
        df = wind_df_f if h == "Wind Risk" else drought_df_f if h == "Drought Risk" else wildfire_df_f
    ...
    for h in hazards:
        ...
        if not filtered.empty:
            ...
            if top_10_clean.empty:
                ...
                hist_fig.update_layout(xaxis_title=metric_name_map[risk_col], yaxis_title="Number of Counties", font=dict(family="Arial", size=12))
                st.plotly_chart(hist_fig, use_container_width=True)

                bar_fig = go.Figure()
                bar_fig.add_trace(go.Bar(x=top_10_clean["County"], y=top_10_clean[risk_col], name=metric_name_map[risk_col], marker_color=colors[h]))
                bar_fig.add_trace(go.Bar(x=top_10_clean["County"], y=top_10_clean["MEAN_low_income_percentage"], name=metric_name_map["MEAN_low_income_percentage"], marker_color="#C0C0C0"))
                bar_fig.update_layout(
                    ...
                    template="plotly_white", font=dict(family="Arial", size=12), xaxis_tickangle=-45
                )
